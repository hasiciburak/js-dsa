  ...
  function updatePageContent() {
    ...
    document.querySelectorAll('[data-i18n]').forEach((el) => {
      ...
      if (value !== undefined && value !== null) {
        // Special handling for footer attribution - add current year
        ...
        // Special handling for footer.message - preserve span element
        if (key === 'footer.message') {
          const span = el.querySelector('span');
          if (span) {
            el.innerHTML = `${value} ${span.outerHTML}`;
            return;
          }
        }

        // Default: Simple text content replacement
    ...
    // Explicitly update footer attribution with year
    const attributionEl =
      document.getElementById('footerAttribution') || document.querySelector('[data-i18n="footer.attribution"]');
    if (attributionEl) {
      const attrValue = getNestedValue(t, 'footer.attribution');
      if (attrValue !== undefined && attrValue !== null) {
        const currentYear = new Date().getFullYear();
        attributionEl.textContent = `© ${currentYear} - ${attrValue}`;
      }
    }
